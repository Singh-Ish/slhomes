<?php
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_instances.php';
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/lib.redux_instances.php';
    }
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_api.php';

        // General helper functions
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_helpers.php';

        // General functions
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_functions.php';
        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.p.php';

        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.thirdparty.fixes.php';

        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_filesystem.php';

        require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/class.redux_admin_notices.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( REALTY_HOUSE_FRAMEWORK . 'core/' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                //     $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'realty-house' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'realty-house' ) . '</a> v' . self::$_version ) . '</span>';
                // }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'realty-house' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'realty-house' );
                }
...
//                    $this->_internationalization();
                    ...
                    if ( $this->args['dev_mode'] == true || Redux_Helpers::isLocalHost() == true ) {
                        require_once REALTY_HOUSE_FRAMEWORK . 'core/core/dashboard.php';
                        new reduxDashboardWidget( $this );

                        if ( ! isset ( $GLOBALS['redux_notice_check'] ) ) {
                            require_once REALTY_HOUSE_FRAMEWORK . 'core/core/newsflash.php';
            ...
            private function _internationalization() {
                 ...
                 * @param string     'realty-house'  text domain
                 */
                ...
                load_plugin_textdomain( 'realty-house', false, $basepath . 'languages' );
            }
            ...
            private function _tracking() {
                if ( file_exists( REALTY_HOUSE_FRAMEWORK . 'core/inc/tracking.php' ) ) {
                    require_once REALTY_HOUSE_FRAMEWORK . 'core/inc/tracking.php';
            ...
            public function show( $opt_name, $default = '' ) {
                ...
                if ( ! is_array( $option ) && $option != '' ) {
                    ...
                    echo esc_html($this->_get_default( $opt_name, $default ));
                }
            }
            // show()
            ...
            public function _enqueue() {
                require_once REALTY_HOUSE_FRAMEWORK . 'core/core/enqueue.php';
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'realty-house' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'realty-house' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'realty-house' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'realty-house' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'realty-house' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'realty-house' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'realty-house' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            private function _register_extensions() {
                $path    = REALTY_HOUSE_FRAMEWORK . 'core/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'realty-house' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'realty-house' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'realty-house' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        require_once REALTY_HOUSE_FRAMEWORK . 'core/core/panel.php';
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'realty-house' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'realty-house' );
